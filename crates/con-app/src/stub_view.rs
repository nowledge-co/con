...
//!
//! This file is selected automatically on non-macOS via the `#[path]`
//! module swap in `main.rs`. When the real Windows/Linux backend
//! (libghostty-vt + ConPTY or forkpty) lands, it will provide a
//! `WindowsTerminalView` / `LinuxTerminalView` and the module swap
//! will point at that instead — the rest of the UI stays the same.
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child("Windows/Linux port — see docs/impl/windows-port.md"),
            )
            .child(
                div()
                    .text_xs()
                    ...
                    .child(
                        "Agent panel, settings, command palette, and the control \
                         socket (\\\\.\\pipe\\con) all work. The terminal surface \
                         will render here when the libghostty-vt + ConPTY backend \
                         lands.",
                    ),
            )
    }
