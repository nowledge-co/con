...
pub fn render_parsed_chat_markdown(
    ...
    if document.render_mode() == ChatMarkdownRenderMode::Compact {
        return render_compact_chat_markdown(blocks, &style);
    }
...
impl ParsedChatMarkdown {
    fn render_mode(&self) -> ChatMarkdownRenderMode {
        if estimated_blocks_cost(&self.blocks) >= 220 {
            ChatMarkdownRenderMode::Compact
        } else {
            ChatMarkdownRenderMode::Rich
        }
    }
}

fn estimated_blocks_cost(blocks: &[MarkdownBlock]) -> usize {
    blocks.iter().map(estimated_block_cost).sum()
}

fn estimated_block_cost(block: &MarkdownBlock) -> usize {
    match block {
        MarkdownBlock::Paragraph(inlines) => 1 + estimated_inline_cost(inlines) / 32,
        MarkdownBlock::Heading { inlines, .. } => 4 + estimated_inline_cost(inlines) / 24,
        MarkdownBlock::CodeBlock { code, .. } => 12 + code.lines().count() * 2 + code.len() / 80,
        MarkdownBlock::BlockQuote(blocks) => 8 + estimated_blocks_cost(blocks),
        MarkdownBlock::List { items, .. } => {
            6 + items
                .iter()
                .map(|item| 4 + estimated_blocks_cost(item))
                .sum::<usize>()
        }
        MarkdownBlock::Table { rows, .. } => {
            let columns = rows.first().map(|row| row.len()).unwrap_or(0);
            18 + rows.len() * columns.max(1) * 6
        }
        MarkdownBlock::Rule => 1,
    }
}

fn estimated_inline_cost(inlines: &[MarkdownInline]) -> usize {
    inlines
        .iter()
        .map(|inline| match inline {
            MarkdownInline::Text(text) | MarkdownInline::Code(text) => text.len(),
            MarkdownInline::Emphasis(children)
            | MarkdownInline::Strong(children)
            | MarkdownInline::Strikethrough(children) => estimated_inline_cost(children),
            MarkdownInline::Link { label, destination } => {
                destination.len() + estimated_inline_cost(label)
            }
...
fn render_compact_chat_markdown(
    ...
    div()
        ...
        .children(
            blocks
                .iter()
                .enumerate()
                .map(|(idx, block)| render_compact_block(block, idx, 0, style)),
        )
        .into_any_element()
}
...
fn render_compact_block(
    ...
    match block {
        ...
        MarkdownBlock::Heading { level, inlines } => div()
            ...
            .into_any_element(),
        MarkdownBlock::CodeBlock { language, code, .. } => render_compact_code_block(
            index,
            language.as_deref(),
            code,
            px(depth as f32 * 12.0),
            style,
        ),
        MarkdownBlock::BlockQuote(blocks) => div()
            .w_full()
            ...
            .child(
                div()
                    ...
                    .child(
                        div().flex().flex_col().gap(px(8.0)).flex_1().children(
                            blocks.iter().enumerate().map(|(nested_idx, nested)| {
                                render_compact_block(nested, nested_idx, depth + 1, style)
                            }),
                        ),
                    ),
            )
        ...
        MarkdownBlock::List {
            ...
            .children(items.iter().enumerate().map(|(item_idx, item_blocks)| {
                ...
                div()
                    ...
                    .child(
                        div().flex().flex_col().gap(px(6.0)).flex_1().children(
                            item_blocks.iter().enumerate().map(|(nested_idx, nested)| {
                                render_compact_block(nested, nested_idx, depth + 1, style)
                            }),
                        ),
                    )
            ...
            .into_any_element(),
        MarkdownBlock::Table { rows, .. } => render_compact_table_block(rows, px(depth as f32 * 12.0), style),
        MarkdownBlock::Rule => div()
            .w_full()
            .pl(px(depth as f32 * 12.0))
            .child(div().w_full().h(px(1.0)).bg(style.rule_color))
            .into_any_element(),
    }
}

fn render_compact_table_block(
    ...
    let lines = compact_table_lines(rows);
    let content = lines.join("\n");
    let mono_style = style.code_text_style();
    let mono_run = mono_style.to_run(content.len());
    ...
    div()
        ...
        .child(
            div()
                ...
                .py(px(9.0))
                .rounded(px(10.0))
                .bg(style.table_header_background.opacity(0.72))
                .font_family(style.theme.mono_font_family.clone())
                .text_size(style.code_font_size)
                .line_height(style.code_line_height)
                .text_color(style.text_color.opacity(0.92))
                .child(StyledText::new(content).with_runs(vec![mono_run])),
        )
        .into_any_element()
}

fn render_compact_code_block(
    index: usize,
    language: Option<&str>,
    code: &str,
    indent: gpui::Pixels,
    ...
    let label = language
        ...
        .unwrap_or("code");
    let mono_style = style.code_text_style();
    let mono_run = mono_style.to_run(code.len().max(1));
            ...
            div()
                ...
                .child(
                    div()
                        ...
                        .child(
                            Clipboard::new(format!("copy-compact-code-block-{index}"))
                ...
                .child(
                    div()
                        .px(px(10.0))
                        .py(px(9.0))
                        .bg(style.code_block_body_background.opacity(0.98))
                        .font_family(style.theme.mono_font_family.clone())
                        .text_size(style.code_font_size)
                        .line_height(style.code_line_height)
                        .text_color(style.text_color.opacity(0.94))
                        .child(StyledText::new(code.to_string()).with_runs(vec![mono_run])),
                ),
        )
        .into_any_element()
}

fn compact_table_lines(rows: &[Vec<Vec<MarkdownInline>>]) -> Vec<String> {
    let column_count = rows.iter().map(|row| row.len()).max().unwrap_or(0);
    let mut widths = vec![0usize; column_count];
    let flattened_rows: Vec<Vec<String>> = rows
        .iter()
        .map(|row| {
            (0..column_count)
                .map(|idx| {
                    let text = row
                        ...
                        .unwrap_or_default()
                        .replace('\n', " ");
                    widths[idx] = widths[idx].max(text.chars().count());
                    text
                })
                .collect()
        })
...
fn inline_plain_text(inlines: &[MarkdownInline]) -> String {
    ...
    append_inline_plain_text(inlines, &mut output);
    output
}

fn append_inline_plain_text(inlines: &[MarkdownInline], output: &mut String) {
    for inline in inlines {
        match inline {
            MarkdownInline::Text(value) | MarkdownInline::Code(value) => output.push_str(value),
            MarkdownInline::Emphasis(children)
            | MarkdownInline::Strong(children)
            | MarkdownInline::Strikethrough(children) => {
                append_inline_plain_text(children, output)
            }
            MarkdownInline::Link { label, .. } => append_inline_plain_text(label, output),
            MarkdownInline::SoftBreak | MarkdownInline::LineBreak => output.push('\n'),
        }
    }
}
