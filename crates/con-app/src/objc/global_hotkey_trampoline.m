...
static id g_con_file_sidebar_monitor = nil;
static uint32_t g_con_focus_files_key_code = UINT32_MAX;
static bool g_con_focus_files_shift = false;
static bool g_con_focus_files_control = false;
static bool g_con_focus_files_alt = false;
static bool g_con_focus_files_command = false;
static con_hotkey_callback_t g_con_focus_files_callback = NULL;
static uint32_t g_con_search_files_key_code = UINT32_MAX;
static bool g_con_search_files_shift = false;
static bool g_con_search_files_control = false;
static bool g_con_search_files_alt = false;
static bool g_con_search_files_command = false;
static con_hotkey_callback_t g_con_search_files_callback = NULL;
...
static BOOL con_event_matches_shortcut(
    NSEvent *event,
    uint32_t key_code,
    bool shift,
    bool control,
    ...
    if (event.type != NSEventTypeKeyDown || key_code == UINT32_MAX) {
        return NO;
    }
    if ((uint32_t)event.keyCode != key_code) {
        return NO;
    }

    NSEventModifierFlags relevant_flags =
        event.modifierFlags &
    ...
    return ((relevant_flags & NSEventModifierFlagShift) != 0) == shift &&
           ((relevant_flags & NSEventModifierFlagControl) != 0) == control &&
           ((relevant_flags & NSEventModifierFlagOption) != 0) == alt &&
           ((relevant_flags & NSEventModifierFlagCommand) != 0) == command;
}

static BOOL con_handle_file_sidebar_shortcut(NSEvent *event) {
    if (g_con_focus_files_callback != NULL &&
        con_event_matches_shortcut(event,
                                   g_con_focus_files_key_code,
                                   g_con_focus_files_shift,
                                   g_con_focus_files_control,
                                   g_con_focus_files_alt,
                                   g_con_focus_files_command)) {
        g_con_focus_files_callback();
        return YES;
    }

    if (g_con_search_files_callback != NULL &&
        con_event_matches_shortcut(event,
                                   g_con_search_files_key_code,
                                   g_con_search_files_shift,
                                   g_con_search_files_control,
                                   g_con_search_files_alt,
                                   g_con_search_files_command)) {
        g_con_search_files_callback();
        return YES;
    }
...
void con_install_file_sidebar_shortcuts(
    uint32_t focus_key_code,
    bool focus_shift,
    bool focus_control,
    bool focus_alt,
    bool focus_command,
    con_hotkey_callback_t focus_callback,
    uint32_t search_key_code,
    bool search_shift,
    bool search_control,
    bool search_alt,
    bool search_command,
    con_hotkey_callback_t search_callback
) {
    g_con_focus_files_key_code = focus_key_code;
    g_con_focus_files_shift = focus_shift;
    g_con_focus_files_control = focus_control;
    g_con_focus_files_alt = focus_alt;
    g_con_focus_files_command = focus_command;
    g_con_focus_files_callback = focus_callback;

    g_con_search_files_key_code = search_key_code;
    g_con_search_files_shift = search_shift;
    g_con_search_files_control = search_control;
    g_con_search_files_alt = search_alt;
    g_con_search_files_command = search_command;
    g_con_search_files_callback = search_callback;

    if (g_con_file_sidebar_monitor != nil) {
        return;
    }

    g_con_file_sidebar_monitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown
                                                                       handler:^NSEvent *(NSEvent *event) {
        if (con_handle_file_sidebar_shortcut(event)) {
            return nil;
        }
        return event;
    }];
...
void con_uninstall_file_sidebar_shortcuts(void) {
    if (g_con_file_sidebar_monitor != nil) {
        [NSEvent removeMonitor:g_con_file_sidebar_monitor];
        g_con_file_sidebar_monitor = nil;
    }

    g_con_focus_files_key_code = UINT32_MAX;
    g_con_focus_files_shift = false;
    g_con_focus_files_control = false;
    g_con_focus_files_alt = false;
    g_con_focus_files_command = false;
    g_con_focus_files_callback = NULL;

    g_con_search_files_key_code = UINT32_MAX;
    g_con_search_files_shift = false;
    g_con_search_files_control = false;
    g_con_search_files_alt = false;
    g_con_search_files_command = false;
    g_con_search_files_callback = NULL;
}
