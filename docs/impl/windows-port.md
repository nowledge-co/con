...
| 3b | Glyph atlas + grid render | HLSL shaders embedded and runtime-compiled via `D3DCompile`. Skyline-packed (`etagere`) BGRA8 glyph atlas (`con-ghostty/src/windows/render/atlas.rs`), glyphs rasterized via Direct2D `DrawText` with grayscale AA. Three-case PUA rasterization (fits-cell / width-overflow with lsb shift / height-overflow with scale-around-centre) for Nerd-Font icons, plus per-slot `PushAxisAlignedClip` + black pre-fill to prevent ClearType fringe bleeding between atlas neighbours. CJK/wide fallback glyphs reserve two-cell atlas slots using `unicode-width`, so DirectWrite fallback faces match the width libghostty-vt reserved in the terminal grid instead of looking clipped followed by a blank spacer cell. D3D11 pipeline (`pipeline.rs`): per-instance IA layout, dynamic instance buffer with `Map(WRITE_DISCARD)`, single `DrawIndexedInstanced(6, cell_count)` per frame — matches Windows Terminal AtlasEngine's architecture. Wide PUA instances are stable-sorted after narrow ones so DX11's in-order per-pixel writes let them win overlap with neighbour backgrounds; the cursor inverse-colour instance is captured pre-sort and pushed post-sort so it always draws last. `vt.rs` uses the `ghostty_render_state` API (row iterator + `row_cells_get_multi` + DIRTY row skip) — not the `grid_ref` path that the upstream header explicitly warns against for render loops. | Real glyph rendering on Windows, runtime-validated on real hardware against oh-my-posh prompt stacks, CJK fallback text, and dense hyphen rules. Postmortem: `postmortem/2026-04-21-windows-atlas-pua-rasterization.md`. ✅ landed |
| 3c | Input + selection | VK_* → xterm escape sequence translation in `host_view`; mouse selection; visible GPUI scrollback scrollbar; wheel/touchpad scrolling through libghostty-vt viewport state when mouse tracking is inactive; alternate-screen mode-1007 wheel gestures translated into cursor keys with the same fractional row accumulator used by primary scrollback; clipboard via OSC 52 + Win32 clipboard. ConPTY child process launch passes the pseudo-console through `PROC_THREAD_ATTRIBUTE_PSEUDOCONSOLE` without inheriting con's unrelated stdout/stderr handles, starts in an explicit absolute cwd when valid, otherwise falls back to the user's home directory and finally `%TEMP%`, and profiling should use `CON_LOG_FILE` instead of shell redirection so the child shell never sees redirected log handles. | Beta-baseline terminal interactivity is landed. Remaining polish belongs in Phase 4 hardening. ✅ largely landed |
