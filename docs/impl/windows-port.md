...
- The terminal still pays a GPU→CPU readback and GPUI re-upload on each
  dirty frame. Recent fixes removed avoidable extra-frame latency and
  several backlog stalls, but Windows Terminal can still feel faster
  until the direct-composition presentation path lands.
