...
One macOS embedding invariant is subtle but important: after GPUI changes split or zoom layout, Con must not position the embedded Ghostty surface from `NSClipView.documentVisibleRect` in the same synchronization pass. Ghostty's native app owns an AppKit `layout()` method for its scroll hierarchy, but Con drives the hierarchy imperatively from GPUI callbacks; AppKit can return a stale visible rect immediately after those mutations. Con therefore computes the surface frame from Con-owned pane bounds plus Ghostty scrollbar state and treats AppKit scroll geometry as an effect, not as the source of truth.
