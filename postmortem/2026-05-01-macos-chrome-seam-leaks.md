...
The first follow-up fix still treated the symptom as a 1–4 px border problem. That was incomplete. During fast hide/show and split changes, AppKit's native terminal view and GPUI's flex layout can be temporarily out of phase. Any exposed pixel outside the exact strip still fell through to the transparent window backing.
