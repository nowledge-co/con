...
- Pane dividers receive a terminal-derived opaque separator color from the workspace. It is precomposed over the terminal background so the final pixel is fully opaque, but it keeps enough foreground tint for users to read split boundaries.
...
- Matching the terminal surface exactly can overcorrect: pane dividers need a visible separator signal. Precompose that signal over the terminal color instead of using a translucent overlay.
