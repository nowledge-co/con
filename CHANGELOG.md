...
- AI summarizer rewritten to use a JSON response shape (`{"label": "...", "icon": "..."}`) with a tolerant parser instead of the original `LABEL|ICON` plain-text format. Reasoning models (Moonshot Kimi K2.6, DeepSeek-R1, …) routinely deliberated for 2–4 KB of chain-of-thought before reaching the answer line and ran out of token budget mid-thought; the JSON-shape preamble + bracket-balanced parser short-circuit the deliberation and gracefully handle markdown code fences (```json …```) the model often emits anyway.
- Periodic AI re-summarization. The pump-driven trigger only fires while output is actively streaming, so once a command finished scrolling the engine never re-asked. Workspace now polls `request_tab_summaries` every 3 s; the engine's per-tab cache + 5 s success budget keep traffic bounded.
- AI labels and icons via a new `TabSummaryEngine` in `con-core`. Each tab's `(cwd, recent commands, OSC title, recent terminal scrollback)` is summarized by the user's already-configured suggestion model (the same one inline shell completions use) into a 1–3 word professional label and one icon from a closed Phosphor set — `terminal`, `code`, `pulse`, `book-open`, `file-code`, `globe`. Gated on `agent.suggestion_model.enabled`; SSH tabs short-circuit to the host name + globe icon with no LLM call. Per-tab cache (keyed on context hash) + 5 s budget per tab keeps traffic bounded. Bad / malformed model output is dropped and the heuristic name takes over — UI never shows a garbage label. Re-asks on every output churn so a tab that just ran `python3 twosum.py` gets named "Twosum" regardless of whether the user typed it directly in the terminal or routed it through the input bar.
